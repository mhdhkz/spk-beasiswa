<div class="row">
	...
	<?php if (isset($_GET["beasiswa"])) {
		...
		$sql = "SELECT
			(SELECT nama FROM mahasiswa WHERE npm=mhs.npm) AS nama,
			(SELECT npm FROM mahasiswa WHERE npm=mhs.npm) AS npm,
			(SELECT tahun_mengajukan FROM mahasiswa WHERE npm=mhs.npm) AS tahun,
			$sqlKriteria
			SUM(
				IF(
						c.sifat = 'max',
	      ...
	      <div class="panel-body">
	          <table class="table table-condensed table-hover">
	              ...
	              <tbody>
					...
					$q = $connection->query("SELECT npm FROM hasil WHERE npm='$row[npm]' AND kd_beasiswa='$_GET[beasiswa]' AND tahun='$row[tahun]'");
					if (!$q->num_rows) {
						$connection->query("INSERT INTO hasil VALUES(NULL, '$_GET[beasiswa]', '$row[npm]', '".$rangking."', '$row[tahun]')");
					} else {
						$connection->query("UPDATE hasil SET nilai='$rangking' WHERE npm='$row[npm]' AND kd_beasiswa='$_GET[beasiswa]' AND tahun='$row[tahun]'");
					}
					?>
					<tr>
						<td><?=$row["npm"]?></td>
