...
<body>
    <div class="container">
                            ...
                            <?php $query = $connection->query("SELECT * FROM beasiswa");
                              if ($query->num_rows > 0):  
                                while ($row = $query->fetch_assoc()): ?>
                                  <li><a href="?page=perhitungan&beasiswa=<?= $row["kd_beasiswa"] ?>"><?= $row["nama"]?> (<?= $row["tahun"]?>)</a></li>
                                <?php endwhile; 
                              else: ?>
        ...
        <footer class="footer">
            <p class="text-center">&copy; <?= date("Y") ?> SPK Beasiswa | Kelompok 3 | All Rights Reserved.</p>
    </div>
